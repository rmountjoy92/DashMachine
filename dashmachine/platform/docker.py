...
class Docker(object):
    ...
    def __init__(self,method, prefix, host, port, api_version, card_type, tls_mode, tls_ca, tls_cert, tls_key):
        self.endpoint = None
        self.method = method
        self.prefix = prefix
        self.host = host
    ...
    def check(self):
        ...
        if rawdata != None:
            if 'message' in rawdata:
                ...
                self.api_version = self.api_version if self.api_version != None else self.max_api_version 
    ...
    def setHtml(self):
        if self.error != None and self.error != '' :
            ...
            <div class="row">
                <div class="col s6">
                    <span class="mt-0 mb-0 theme-primary-text font-weight-700" style="font-size: 36px">"""+ img_tls + img_warnings +"""</h3>
                </div>
                <div class="col s6 right-align">
                    <img height="48px" src="static/images/apps/docker.png" alt="Docker">
                </div>
...
class Platform:
    def __init__(self,*args,**kwargs):
        ...
        self.docker = Docker(self.method, self.prefix, self.host, self.port, self.api_version, self.card_type, self.tls_mode, self.tls_ca, self.tls_cert, self.tls_key)
    ...
    def process(self):
        ...
        if self.card_type == "Custom":
            return render_template_string(
                self.docker.getHtml(),
                **self.docker.__dict__
            )
        else:
            return render_template_string(
                self.value_template, **self.docker.__dict__
            )
